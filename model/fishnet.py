...
### https://github.com/kevin-ssy/FishNet/blob/master/models/fish_block.py
    ...
    def _fish_forward(self, all_feat):
        ...
        while stg_id < self.depth:
            ...
            if stg_id == self.depth:
                ...
                return score_feat, score

    def forward(self, x):
        all_feat = [None] * (self.depth + 1)
        all_feat[0] = x
        score_feat, score = self._fish_forward(all_feat)
        return all_feat[4], score_feat, score
...
class FishNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv3(x)
        x = self.pool1(x)
        inter_feat, score_feat, score = self.fish(x)
        # 1*1 output
        out = score.view(x.size(0), -1)

        return inter_feat, score_feat, out


def fish(**kwargs):
    return FishNet(Bottleneck, **kwargs)

# https://github.com/kevin-ssy/FishNet/blob/master/models/net_factory.py
